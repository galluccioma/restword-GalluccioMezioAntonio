...
export default function CreateProfile() {
  ...
  const calculateProfileStrength = () => {
    ...
    // Personal info
    if (formData.personal.firstName && formData.personal.lastName) strength += 1
    ...
    if (formData.preferences.jobType && formData.preferences.jobType.length > 0) strength += 1
    total += 2
      ...
      <div className="max-w-5xl mx-auto rounded-lg shadow-md">
          ...
          {profileStrength < 50 && (
            <div className="mt-4 p-4 bg-gray-50 rounded-lg border">
              <p className="text-sm font-medium mb-2">
                <span className="text-red-500">Forza del profilo: Debole</span>
              </p>
              <ul className="text-sm space-y-2">
                <li>
                  Aggiungi una foto professionale per mostrare il volto dietro le tue creazioni culinarie (+5 punti)
                </li>
                <li>
                  Aggiungi più esperienze lavorative per mostrare i diversi ristoranti in cui hai affinato le tue
                  abilità (+10 punti)
                </li>
              </ul>
            </div>
        ...
        <Card>
          <CardContent className="pt-6">
            ...
            {currentStep === 4 && (
              <PreferencesForm data={formData.preferences} updateData={(data) => updateFormData("preferences", data)} />
            )}

            {currentStep === 5 && <CvUploadForm data={formData.cv} updateData={(data) => updateFormData("cv", data)} />}
